...
frag.size <- args[2]
out.dir <- args[3]
# samples.csv <- 'H3K4_analysis/H3K4me3_samples.csv'
# frag.size <- 150
...
write.table(as.data.frame(diff.full.gr), file=paste0(out.dir, 'H3K4_peaks.csv'), sep=',', 
  quote=F, row.names=F, col.names=T)
write.table(as.data.frame(diff2.full.gr), file=paste0(out.dir, 'H3K4_peaks_in2.csv'), sep=',', 
  quote=F, row.names=F, col.names=T)

# Write differentially bound GRanges objects to csv
write.table(as.data.frame(diff.gr), file=paste0(out.dir, "H3K4_diff_bound.csv"), 
  quote=F, sep=",", row.names=F, col.names=T)
write.table(as.data.frame(diff2.gr), file=paste0(out.dir, "H3K4_diff_bound_in2.csv"), 
  quote=F, sep=",", row.names=F, col.names=T)
...
dba.plotPCA(counts.dba, DBA_CONDITION)
dev.off()

pdf(paste0(out.dir, 'pca2.pdf'))
dba.plotPCA(counts2.dba, DBA_CONDITION)
dev.off()

# PCA using just differentially bound sites
pdf(paste0(out.dir, 'pca_diff.pdf'))
dba.plotPCA(counts.dba, DBA_CONDITION, contrast=1)
dev.off()

pdf(paste0(out.dir, 'pca_diff2.pdf'))
dba.plotPCA(counts2.dba, DBA_CONDITION, contrast=1)
dev.off()
